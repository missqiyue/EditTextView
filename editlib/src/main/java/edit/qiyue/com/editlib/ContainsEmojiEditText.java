            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (!resetText) {
                    if (count >= 2) {//表情符号的字符长度最小为2
                        ...
                        if (length > cursorPos && length >= cursorPos + count) {
                            input = s.subSequence(cursorPos, cursorPos + count);
                        } else {
                            if (length <= cursorPos) {
                                input = s.subSequence(0, length);
                            } else if (length > cursorPos && length < cursorPos + count) {
                                input = s.subSequence(cursorPos, length);
                            }
                        }
